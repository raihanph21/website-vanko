...
<meta charset="utf-8">
    ...
    <title>Kasir Page | Pesanan</title>
    <link rel="stylesheet" type="text/css" href="../skripsi/css/styleKasirPesanan.css">
...
<body>
    <div class="row">
        <div class="col-3" id="sticky-sidebar">
            <div class="sticky-top">
                <div class="nav flex-column">
                    ...
                    <li href="#_" class="nav-link text-center"><a href="kasirPage.php">Stok</a></li>
                    <li href="#_" class="nav-link text-center"><a href="kasirLaporan.php">Laporan Keuangan</a></li>
                    <li href="#_" class="nav-link text-center active"><a href="kasirPesanan.php">Pesanan</a></li>
        ...
        <div class="col-9">
            ...
            <!-- Filter Form -->
            <form method="GET" action="kasirPesanan.php">
                <div class="row">
                    <div class="col">
                    ...
                    <div id="btnSubmit" class="col">
                        ...
                        <a id="bPesananOffline" class="btn btn-primary" href="kasirTambahPesananOffline.php">Input Pesanan Offline</a>
                    </div>
                </div>
            </form>
            ...
            <table class="table table-hover text-center table-bordered table-striped">
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr onclick="window.location.href='kasirDetailPesanan.php?id_penjualan=<?= $row['id_penjualan'] ?>'">
...
<div class="mt-3">
    <nav aria-label="Pagination">
            <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                <a class="page-link" href="kasirPesanan.php?start_date=<?= htmlspecialchars($start_date) ?>&end_date=<?= htmlspecialchars($end_date) ?>&limit=<?= $limit ?>&page=<?= $page - 1 ?><?= $show_failed ? '&show_failed=1' : '' ?>" aria-label="Previous">
            ...
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="kasirPesanan.php?start_date=<?= htmlspecialchars($start_date) ?>&end_date=<?= htmlspecialchars($end_date) ?>&limit=<?= $limit ?>&page=<?= $i ?><?= $show_failed ? '&show_failed=1' : '' ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <li class="page-item <?= $page >= $total_pages ? 'disabled' : '' ?>">
                <a class="page-link" href="kasirPesanan.php?start_date=<?= htmlspecialchars($start_date) ?>&end_date=<?= htmlspecialchars($end_date) ?>&limit=<?= $limit ?>&page=<?= $page + 1 ?><?= $show_failed ? '&show_failed=1' : '' ?>" aria-label="Next">
